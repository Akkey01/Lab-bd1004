...
```bash
# 1. Verify the Hadoop streaming JAR exists
ls /usr/lib/hadoop-mapreduce/hadoop-streaming*.jar

# 2. Create a unique output directory name using a timestamp
...
The JAR location varies depending on the cluster setup. **Don't panic — search for it:**

```bash
find / -name "hadoop-streaming*.jar" 2>/dev/null
```

This searches the entire filesystem. `2>/dev/null` hides "permission denied" noise. You might see multiple results like:
...
**Don't worry — they are all the same thing.** The Hadoop Streaming JAR is the bridge between your Python code and Hadoop's Java engine. Hadoop is written in Java, but your mapper/reducer are Python scripts. The streaming JAR tells Hadoop: "pipe each input line to this Python script's stdin, and collect its stdout as output." That's how `mrjob` talks to Hadoop under the hood.

The different filenames are just different versions or symlinks (shortcuts). The one without a version number (e.g. `hadoop-streaming.jar`) is usually a symlink pointing to the versioned one. **Any of them will work — just pick one.**

**Pro tip:** Save it to a variable so you don't have to retype it:

```bash
STREAMING_JAR=$(find / -name "hadoop-streaming*.jar" 2>/dev/null | head -1)
echo $STREAMING_JAR
# Then use $STREAMING_JAR in your command:
python3 src/mr_sales_per_category.py data/shopping_data_200.csv \
    -r hadoop \
